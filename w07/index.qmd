...
format:
  revealjs:
    df-print: kable
    footer: "DSAN 5300 Week 7: {{< var w07.footer >}}"
...
| **Break!** | 7:50pm | 8:00pm | |
| | 8:00pm | 8:30pm | [Quiz 7 Review &rarr;](#quiz-7-review) |
...
# Segments $\rightarrow$ Splines {data-stack-name="Splines" .smaller .crunch-title .table-va}
...
discontinuous_plot <- prod_df |> ggplot(aes(x=x, y=y, group=segment)) +
  geom_point(size=0.5) +
...
## $\leadsto$ Splines: Continuity Constraints {.text-65 .crunch-title .title-11 .crunch-quarto-figure .crunch-math .math-80 .crunch-quarto-layout-panel .crunch-img .crunch-ul .crunch-p}

We "pay" in units of **degrees of freedom** for each constraint!

:::: {layout="[70,30]" layout-valign="center"}
::: {#unconstrained-cubic}

<i class='bi bi-0-circle'></i> Two unconstrained cubics: $\text{df} = 8$

$$
f(x_i) = \begin{cases}
\beta_{00} + \beta_{10}x_i + \beta_{20}x_i^2 + \beta_{30}x_i^3 &\text{if }x_i < c \\
\beta_{01} + \beta_{11}x_i + \beta_{21}x_i^2 + \beta_{31}x_i^3 &\text{if }x_i \geq c
\end{cases}
$$

:::
::: {#unconstrained-cubic-plot}

```{r}
#| label: quad-naive-spline-again
#| fig-align: center
discontinuous_plot
```
...
<i class='bi bi-1-circle'></i> Continuity constraint: $f_0(c) = f_1(c)$, $\text{df} = 7$

$$
\begin{align*}
f_0(c) = f_1(c) \iff &\beta_{00} + \beta_{10}x_i + \beta_{20}x_i^2 + \beta_{30}x_i^3 \\
&= \beta_{01} + \beta_{11}x_i + \beta_{21}x_i^2 + \beta_{31}x_i^3
\end{align*}
$$

:::
::: {#cont-constraint-plot}
...
<i class='bi bi-2-circle'></i> Derivative continuity constraint: $f'_0(c) = f'_1(c)$, $\text{df} = 6$
...
![](images/ditto.png){fig-align="center" width="160"}
...
![](images/wigglytuff.jpg){fig-align="center" width="150"}
...
# Quiz 7 Review {data-stack-name="Quiz 7"}

## Optimal vs. Greedy Subset Selection
