...
format:
  revealjs:
    df-print: kable
    footer: "DSAN 5300 Week 6: {{< var w06.footer >}}"
...
## Top Secret Equivalent Forms {.smaller .crunch-title .title-11 .math-90 .crunch-math}
...
Best Subset Selection *(Double-Secret; Computationally Infeasible)*

$$
\boldsymbol\beta^*_{\text{lasso}} = \arg \left\{\min_{\boldsymbol\beta}\left[ \frac{1}{N}\sum_{i=1}^{N}(\widehat{y}_i(\boldsymbol\beta) - y_i)^2 \right] \; \text{ subject to } \; \| \sum_{j=1}^{p}\mathbb{1}[\beta_j \neq 0] \leq s \right\}
$$
